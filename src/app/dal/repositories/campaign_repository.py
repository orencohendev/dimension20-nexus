...
class CampaignRepository:
    ...
    async def get_all() -> List[Campaign]:
        ...
        async with AsyncSession(db_engine.get_async_engine()) as session:
            result = await session.execute(select(CampaignModel).options(
                    selectinload(CampaignModel.reviews)  # EAGER LOAD
                ))
            db_models = result.scalars().all()

            return [Campaign.model_validate(db_model, from_attributes=True) for db_model in db_models]
    ...
    async def get_by_id(campaign_id: UUID) -> Optional[Campaign]:
        ...
        async with AsyncSession(db_engine.get_async_engine()) as session:
            ...
            return Campaign.model_validate(db_model, from_attributes=True) if db_model else None
